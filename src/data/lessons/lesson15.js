...
model.add(Dense(units=1, activation='sigmoid'))</code></pre>
    ...
    <pre><code>from keras.layers import Dense, BatchNormalization, Activation
...
model.add(Activation('relu'))      # 3. Активация</code></pre>
    ...
    <pre><code>model.add(Dense(128, activation='relu'))</code></pre>

    <details>
      <summary>Жауапты көру (Шешімі)</summary>
      <pre><code>model.add(Dense(128))
